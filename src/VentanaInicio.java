...
public class VentanaInicio extends javax.swing.JFrame 
{
    ...
    public void añadeDatos(ResultSet rs)//Con este método se rellenará la tabla con los datos obtenidos de la consulta
    {
        ...
        try 
        {
            if(rs!=null)
            {
                System.out.println("AÑADE DATOS");
                int x = 0;
                while(rs.next())
                {
                    for(int i=0;i<ncampos;i++)
                    {
                        ...
                        if(valores[i].contains("oracle.sql"))
                        {
                            String fktabla = gestor.obtenerFK(tablaElegida, tcampos[i]);
                            if(!fktabla.equals("")&&!fktabla.equals("null")&&tcampos.length>0)
                            {
                                String pk = gestor.obtenerPK(fktabla);
                                String fkdatos[] = gestor.realizaConsultaREF(tablaElegida, tcampos[i], pk, fktabla);
                                String camposFK[] = gestor.obtenerTitulos(fktabla, "campos");
                                for(int k = 0; k<camposFK.length; k++)
                                {
                                    if(camposFK.length>1 && camposFK[k].equals(pk))
                                    {
                                        String fkdatos2[] = gestor.realizaConsultaREF(tablaElegida, tcampos[i], camposFK[k+1], fktabla);
                                        fkdatos[x] = fkdatos[x] + " - " + fkdatos2[x]; 
                                        break;
                                    }
                                }
                                valores[i] = fkdatos[x];
        ...
        if(eleccionValor.getItemCount()<1)
        {
            for(int i=0; i < eleccionValores.length; i++)
            {
                    ...
                    //Si el dato a mostrar tiene pk, se le añadirá por delante de este con un | para diferenciarlos
                {
                    ...
                    {
                        String pk = gestor.obtenerPK(fktabla);
                        String fkdatos[] = gestor.realizaConsultaREF(tablaElegida, tcampos[0], pk, fktabla);
                        String camposFK[] = gestor.obtenerTitulos(fktabla, "campos");
                        for(int k = 0; k<camposFK.length; k++)
                        {
                            if(camposFK.length>1 && camposFK[k].equals(pk))
                            {
                                String fkdatos2[] = gestor.realizaConsultaREF(tablaElegida, tcampos[0], camposFK[k+1], fktabla);
                                if(eleccionCampo2.getItemCount()>1)
                                {
                                    fkdatos = gestor.realizaConsultaREF(tablaElegida, campoElegido, pk, fktabla);
                                    fkdatos2 = gestor.realizaConsultaREF(tablaElegida, campoElegido, camposFK[k+1], fktabla);
                                }
                                     
                                fkdatos[i] = fkdatos[i] + " - " + fkdatos2[i]; 
                                break;
                            }
                        }
                        eleccionValores[i] = fkdatos[i];
                    }
                    if(!pktabla.equals("")&&tcampos.length>0)
                    {
                        if(eleccionCampo2.getItemCount()>1)
